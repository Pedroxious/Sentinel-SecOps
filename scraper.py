...
<body>
            ...
            <section class="chat-section" id="chat-section">
                ...
                <div class="chat-input-area">
                    ...
                    <div class="input-footer">
                        ...
                        <button type="button" onclick="promptApiKey()" class="api-key-btn" style="background:transparent;border:none;color:var(--text-muted);font-size:0.72rem;cursor:pointer;display:inline-flex;align-items:center;gap:4px;padding:2px 6px;border-radius:4px;transition:all 0.15s;" title="Configurar ou alterar chave de API do Gemini">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12"><path d="M21 2l-2 2m-1.5 1.5L14 9l-1.5-1.5L10 10l-1.5-1.5L7 10 2 15v7h7l5-5 1.5 1.5 2.5-2.5-1.5-1.5 3.5-3.5 1.5 1.5L22 13l-1-11z"/></svg>
                            <span id="api-key-status">Chave API</span>
                        </button>
                    </div>
                </div>
    ...
    try {{
        const injectedKey = atob("{api_key_b64}");
        if (!API_KEY && injectedKey && injectedKey.startsWith("AIzaSy")) {{
            API_KEY = injectedKey;
        }}
    }} catch(e) {{}}
    const MAX_QUOTA = 20;
...
Como posso auxiliar na segurança hoje?`;
    ...
    function promptApiKey() {{
        ...
        const userKey = prompt("Insira sua chave de API do Google Gemini (AI Studio):", current);
        if (userKey !== null) {{
    ...
    function updateApiKeyStatus() {{
        ...
        if (!el) return;
        const hasKey = !!(API_KEY && API_KEY.startsWith("AIzaSy"));
        el.textContent = hasKey ? "Chave Ativa" : "Configurar Chave";
        el.style.color = hasKey ? "var(--green)" : "var(--text-muted)";
    }}

    /* ═══════════════════════════════════════════════════════════
       CHAT SUBMIT & GEMINI API CALL
    ═══════════════════════════════════════════════════════════ */
    async function handleSubmit(e) {{
        ...
        if (!API_KEY) {{
            const userKey = prompt("Insira sua chave de API do Gemini (AI Studio) para respostas completas em tempo real:", localStorage.getItem("GEMINI_API_KEY") || "");
        ...
        try {{
            if (!API_KEY) {{
                const localReply = generateLocalSocResponse(message);
                appendBotBubble(localReply);
            ...
            for (const model of GEMINI_MODELS) {{
                try {{
                    ...
                    if (res.status === 401 || res.status === 403 || res.status === 400) {{
                        ...
                        API_KEY = "";
                        updateApiKeyStatus();
                        showError("Chave de API inválida ou sem permissão. Clique em 'Configurar Chave' no rodapé para atualizar sua chave Gemini AI Studio.");
                        break;
                    }}
            ...
            if (!reply) {{
                if (lastError) console.error("Aeris API Error:", lastError);
    ...
    function generateLocalSocResponse(query) {{
        ...
        if (q.includes("mitre") || q.includes("t1190")) {{
            return "A técnica **MITRE ATT&CK T1190** (*Exploit Public-Facing Application*) descreve invasões através da exploração de falhas em softwares expostos à Internet (como portas 80/443 de servidores web, portais corporativos ou VPNs). Recomendações: aplicar patches imediatos, isolar endpoints via WAF e habilitar monitoramento de anomalias nos logs HTTP.";
        }}
        if (q.includes("ransomware")) {{
            return "No monitoramento diário do **Sentinel SecOps**, os alertas de vulnerabilidades são correlacionados diretamente com catálogos CISA KEV e bases de ransomware ativo. Para mitigar riscos: garanta backups offline imutáveis, autenticação multifator (MFA) em todos os acessos e segmentação estrita entre redes de TI e TO.";
        }}
        if (q.includes("cve") || q.includes("vulnerab") || q.includes("hoje") || q.includes("painel") || q.includes("status")) {{
            return "O motor de Threat Intelligence do Sentinel SecOps monitora continuamente a stack tecnológica corporativa (Apache, Windows Server, VMware ESXi, PostgreSQL, nginx, Python, Node.js). O sumário executivo e as métricas do ciclo atual podem ser consultados no painel logo abaixo.";
        }}
        if (q.includes("ola") || q.includes("olá") || q.includes("oi") || q.includes("ajuda")) {{
            return "Olá! Sou **Aeris**, analista virtual de inteligência cibernética da Pedroxious Lab. Posso ajudar analisando CVEs, táticas do MITRE ATT&CK, vetores de exploração ou orientando sobre as melhores práticas de mitigação. Como posso auxiliar na defesa hoje?";
        }}
        return "Analisando sua solicitação sob a ótica de segurança cibernética: para diagnósticos avançados em tempo real com o modelo do Google Gemini, você pode configurar uma chave ativa do AI Studio clicando em **Configurar Chave** no rodapé do chat. Em que mais posso ajudar?";
    }}
